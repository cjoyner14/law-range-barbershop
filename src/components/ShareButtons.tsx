...
export default function ShareButtons({ variant = 'dark' }: ShareButtonsProps) {
  ...
  const btnClass =
    variant === 'dark'
      ? 'w-9 h-9 rounded-full bg-cream/10 flex items-center justify-center hover:bg-gold/20 transition-colors'
      : 'w-9 h-9 rounded-full bg-charcoal/10 flex items-center justify-center hover:bg-gold/20 transition-colors';

  const iconClass = variant === 'dark' ? 'w-4 h-4 text-cream' : 'w-4 h-4 text-charcoal';
